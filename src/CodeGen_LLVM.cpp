...
void CodeGen_LLVM::begin_func(LinkageType linkage, const std::string &name,
    ...
    function_uses_streaming_store = false;
...
void CodeGen_LLVM::end_func(const std::vector<LoweredArgument> &args) {
    ...
    if (function_uses_streaming_store) {
        IRBuilderBase::InsertPoint here = builder->saveIP();
        builder->SetInsertPoint(destructor_block->getTerminator());
    ...
    if (ramp && is_const_one(ramp->stride) && !emit_atomic_stores) {  // Dense vector store
        ...
        for (int i = 0; i < store_lanes; i += native_lanes) {
            ...
            if (op->is_streaming) {
                ...
                function_uses_streaming_store = true;
            }
        }
...
void CodeGen_LLVM::visit(const ProducerConsumer *op) {
    ...
    if (op->is_producer) {
        bool has_streaming_store = false;
        visit_with(op->body, [&](auto *, const Store *store) {
            has_streaming_store |= store->is_streaming;
        });
        if (has_streaming_store) {
            emit_streaming_store_fence();
            function_uses_streaming_store = false;
        }
    }
